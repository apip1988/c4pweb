...
/*
|--------------------------------------------------------------------------
| Web Routes - SISTEM AMOPPP (VERSI RESTORE DATA STATISTIK & DIREKTORI)
|--------------------------------------------------------------------------
*/
...
// --- 6. PENGURUSAN DOKUMEN (FIX: senarai_stats pluck error) ---
Route::get('/pengurusan-dokumen-utama', function () { 
    // Kita tarik data dari table statistik_utama supaya .pluck() kat Blade boleh jalan
...
// Route simpan statistik (dari form baris 8)
Route::post('/admin/profil/store', function (\Illuminate\Http\Request $request) {
    foreach ($request->stats as $kategori => $jumlah) {
        DB::table('statistik_utama')->updateOrInsert(
            ['kategori' => $kategori],
            ['jumlah' => $jumlah, 'updated_at' => now()]
        );
    }
    return back()->with('success', 'Statistik Dashboard berjaya dikemaskini!');
});
...
// --- 10. DIREKTORI & PROFIL (DIPULIHKAN SEPENUHNYA) ---
